...
.equ lcd_display_on = 0b00001111	   ;display on(DB2 = 1), cursor on(DB1 = 1), blink on(DB0 = 1)
.equ lcd_entry_set = 0b00000110	     ;increase address counter(DB1 = 1), no shift(DB0 = 0)
.equ lcd_clear_display = 0b00000001	 ;clear display, cursor moves to the upper left corner
;command constant
...
main:
	...
	cpi counter, 17	;if counter eq 9
	breq newLine
	cpi counter, 33
	breq clean_screen
	call lcd_data
...
lcd_command:
	out PORTC, r23	;write command data into port F
	nop
...
lcd_data:
	out PORTC, r23	;write data value into port F
...
lcd_wait:
	push r23
	clr r23
	out DDRC, r23	;port F is input mode
	out PORTC, r23	;clear port F
...
sleep_1s:
 ...
 readKeypad:
	;push row	;value for row number
	;push col	;value for col number
	;push rmask	;mask for current row during scan
	;push cmask	;mask for current colum during scan
	;push temp
	;push temp1
	...
	convert_end:
		rcall sleep_1s
		mov r23, temp
		;pop temp1	;value for row number
		;pop temp	;value for col number
		;pop cmask	;mask for current row during scan
		;pop rmask	;mask for current colum during scan
		;pop col
		;pop row
		ret
