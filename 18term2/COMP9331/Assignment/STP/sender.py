...
class STPSender:
    ...
    def print_snd_log(self,event, timeOfevent, typeOfPacket, seqNum, NbOfData, ackNum):
        print(f"{event:8}\t{timeOfevent:4,.2f}\t{typeOfPacket:4}\t{seqNum:4}\t{NbOfData:4}\t{ackNum:4}", file = self.log)
    ...
    def print_rcv_log(self, event, time_for_happen, type_of_packet, seq_num, number_of_bytes, ack_num):
        ...
        event:
            ...
            DA(Duplicate ACK)
        '''
        line = f"{event:6}\t{time_for_happen:4,.2f}\t{type_of_packet:4}\t{seq_num:4}\t{number_of_bytes:4}\t{ack_num:4}\n"
        self.log.write(line)
